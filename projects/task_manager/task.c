...
int task_print(const task_t *t)
{
    int id, done;
    char name[MAXBUF], desc[MAXBUF];

    if (!t)
        return 1;

    if (task_get_id(t, &id) || task_get_taskname(t, name) ||
        task_get_desc(t, desc) || task_get_done(t, &done))
        return 2;

    printf("----------------------------------------\n");
    printf(" [%s] ID: %03d | NOME: %s\n", (done ? "X" : " "), id, name);
    printf(" DESC: %s\n", desc);
    printf("----------------------------------------\n");

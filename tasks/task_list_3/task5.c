// 5) Dado o exercício 4 calcule o percentual de jovens de cada sexo em cada
// modalidade.
  ...
  for (i = 0; i < MAXSPORT; ++i)
    for (y = 0; y < 2; ++y)
      printf ("O percentual de jovens do gênero %s "
              "no esporte %s é %.2f.\n",
              (y) ? "Masculino" : "Feminino", sports[i], percs[i][y]);
...
int
gen_percs (int gend[], int sport[], float percs[][2])
{
  size_t i;

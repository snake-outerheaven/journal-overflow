...
int main()
{
        ...
        int tsMaiorV = 0, locMaiorV = 0, valMaiorV = -1, dayMaiorV = 0, valMaiorVS = 0; // valMaiorV setado para -1 pois é um valor coringa útil para comparações entre iterações
        ...
        float percNt15Int,
            avgDayTot, avgDayTs1, avgDayTs0;

        setlocale(LC_ALL, "pt_BR.UTF-8");
        // configurando locale do executável, pois o Windows coloca a pagina de código sem unicode.
        srand(time(0));
        ...
        while (cont < N)
        {
                ...
                sprintf(code, "NT%d%c", codenum, ts ? 'T' : 'F'); // geração do código de acordo com ts.
                ...
                if (val > valMaiorV)
                {
                        ...
                        locMaiorV = loc;
                        dayMaiorV = day;
                        valMaiorVS = val; // valor que é salvo em cada iteração para ser imprimido no final.
                }
                ...
                if (loc && day < 15)
                {
                        totLoc115++; // contagem de nts de loc interestadual e dias menor q 15
                }

                printf("%s\t\t%s\t\t%s\t\t%s\t\t%d\t\t%d\n",
                       code, mes, ts ? "Transporte" : "Fretamento", loc ? "Interestadual" : "Estadual", day, val);

                cont++, codenum++;
        }

        percNt15Int = totLoc115 / N * 100; // percentual de loc 1 com day < 15 entre todas as notas
        ...
        printf("Percentual de nts com duração menor q 15 dias e interestaduais: %.2f%%\n", percNt15Int);
        ...
        puts("Código\t\tMês\t\tTipo\t\tLocal\t\tDias\t\tValor\t\t");
        printf("%s\t\t%s\t\t%s\t\t%s\t\t%d\t\t%d\n",
               codeMaiorV, mesMaiorV, tsMaiorV ? "Transporte" : "Fretamento", locMaiorV ? "Interestadual" : "Estadual", dayMaiorV, valMaiorVS);
